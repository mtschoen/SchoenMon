...
> **RESUME HERE (wrapped 2026-06-12 mid-Task 4):**
> - Done + committed: Phase 0 ALPHA WORKS (`aab598b`); Task 3 holotable scale (`77b80d7`). Grab fly-away persists - filed as Task 3b below. All session commits are UNPUSHED.
> - **Task 4 is in flight and UNCOMMITTED.** The working tree holds three layers in `ui/xr/`: (a) the specced heat-map shader in `TerrainGLRenderer.kt` (plan-compliance reviewed, exact match), (b) a v-flip fix in `VERT_SRC` (`gl_Position` y = `1.0 - aGridPos.y * 2.0`) - root cause was GL bottom-up vs Surface consumer top-down sampling, user-confirmed that colors now TRACK the geometry, and (c) a clearly-fenced TEMP orientation probe in `TerrainSurface.kt` (synthetic heightmap: RED stripe = row 0 = nearest lane (+z), GREEN stripe = row 15 = farthest, navy-to-cyan time ramp oldest-to-newest; the restore line is in the comment fence).
> - Remaining symptom: "tracks, but mirrored/shifted" (time-axis flip or lane offset, undetermined). The probe APK was BUILT but NEVER INSTALLED (adb dropped). Next session: reconnect adb (wake headset, toggle Wireless debugging, `adb mdns services`, disconnect any stale duplicate device entry - see AGENTS.md section 5), run the build/deploy loop to install the probe, user looks ONCE (red stripe near or far? ramp bright end left or right?), apply the final orientation fix (consider folding in texel-center sampling, (row+0.5)/ROWS, if a half-lane offset is seen), STRIP the probe, then Task 4 steps 3-4 (verify + commit).
> - Perf note for Task 8: tick cost rose to ~5.8ms at holotable scale (was ~2.3ms; geometry unchanged, likely compositor-side setShape cost) - re-check against the <5ms 30fps budget; fall back to 15fps per the plan if needed.
> - Debug evidence (session `218237b6-...`): 428-tick DIAG capture proved heightmap uploads change every tick and renders consume them (staleness ruled out); decompiled `SurfaceFeatureImpl` (alpha15) shows `setShape` reuses the Impress node, no surface recreation.
